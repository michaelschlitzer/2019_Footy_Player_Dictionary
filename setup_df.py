...
class Home_Away:
	...
	def home_away_setup(self):
		...
		fixtures = [self.home_fixture, self.away_fixture]
		flists = []

		# return self.home_fixture

		for f in fixtures:

			f.replace({'Greater Western Sydney': 'GWS'}, inplace = True)
			f['Game'] = f['Game'].str.lstrip(string.digits)
			f.sort_values(by = ['Year', 'Round', 'Game'], axis = 0, ignore_index = True, inplace = True)

		home = pd.merge(self.home_fixture, self.master_df, how = 'left', left_on = ['Year', 'Home', 'Round'], right_on = ['Season', 'Team', 'Round'])
		away = pd.merge(self.away_fixture, self.master_df, how = 'left', left_on = ['Year', 'Away', 'Round'], right_on = ['Season', 'Team', 'Round'])	

		lists = [home, away]

		for l in lists:
			l.drop(['Team', 'Season'], axis = 1, inplace = True)
		...
		(info['Net Score'], info['Intrastate'], info['Same / Neutral Venue']) = ((info['Home Score'] - info['Away Score']),
			np.where(info['Home State'] == info['Away State'], 0, 1),
		...
		# Here I reconstruct and finalize the information portion of the dataframe, 
		# merge it with the PI, and then reorder it.
		...
		d_cols = [col for col in self.master_df.columns if col.startswith('D-')]
		f_cols = [col for col in self.master_df.columns if col.startswith('F-')]
		m_cols = [col for col in self.master_df.columns if col.startswith('M-')]
		r_cols = [col for col in self.master_df.columns if col.startswith('R-')]
		...
		hdaf['HDAF'+'-TM'] = hdaf['HDAF'+'-MK'] + hdaf['HDAF'+'-CM'] + hdaf['HDAF'+'-MI']
		hdaf['HDAF'+'-TP'] = hdaf['HDAF'+'-CP'] + hdaf['HDAF'+'-UP']
		hdaf['HDAF'+'-TT'] = hdaf['HDAF'+'-FA'] + hdaf['HDAF'+'-CG']

		hfad['HFAD'+'-TM'] = hfad['HFAD'+'-MK'] + hfad['HFAD'+'-CM'] + hfad['HFAD'+'-MI']
		hfad['HFAD'+'-TP'] = hfad['HFAD'+'-CP'] + hfad['HFAD'+'-UP']
		hfad['HFAD'+'-TT'] = hfad['HFAD'+'-FA'] + hfad['HFAD'+'-CG']
		...
		home_away_net_complete = home_away_net_complete[['Home', 'Away', 'Season', 'Game', 'Round', 'D-DI', 'D-KI', 
		'D-MK', 'D-HB', 'D-GL', 'D-BH', 'D-HO', 'D-TK', 'D-RB', 'D-IF', 'D-CL', 'D-CG', 'D-FF', 'D-FA', 'D-CP', 'D-UP', 
		'D-CM', 'D-MI', 'D-1%', 'D-BO', 'D-AMG','D-GA', 'D-Height', 'D-Weight', 'D-Age', 'F-DI', 'F-KI', 'F-MK', 'F-HB', 
		'F-GL', 'F-BH', 'F-HO', 'F-TK', 'F-RB', 'F-IF', 'F-CL', 'F-CG', 'F-FF', 'F-FA', 'F-CP', 'F-UP', 'F-CM', 'F-MI', 
		'F-1%', 'F-BO', 'F-AMG','F-GA', 'F-Height', 'F-Weight', 'F-Age', 'M-DI', 'M-KI', 'M-MK', 'M-HB', 'M-GL', 'M-BH', 
		'M-HO', 'M-TK', 'M-RB', 'M-IF', 'M-CL', 'M-CG', 'M-FF', 'M-FA', 'M-CP', 'M-UP', 'M-CM', 'M-MI', 'M-1%', 'M-BO', 
		'M-AMG','M-GA', 'M-Height', 'M-Weight', 'M-Age', 'R-DI', 'R-KI', 'R-MK', 'R-HB', 'R-GL', 'R-BH', 'R-HO', 'R-TK', 
		'R-RB', 'R-IF', 'R-CL', 'R-CG', 'R-FF', 'R-FA', 'R-CP', 'R-UP', 'R-CM', 'R-MI', 'R-1%', 'R-BO', 'R-AMG','R-GA', 
		'R-Height', 'R-Weight', 'R-Age', 'HFAD-TM', 'HFAD-TP', 'HFAD-FF', 'HFAD-TT', 'HFAD-Height', 'HFAD-Weight', 
		'HFAD-Age', 'HDAF-TM', 'HDAF-TP', 'HDAF-FF', 'HDAF-TT', 'HDAF-Height', 'HDAF-Weight', 'HDAF-Age', 'Intrastate',
